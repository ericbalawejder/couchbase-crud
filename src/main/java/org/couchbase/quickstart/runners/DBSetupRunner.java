...
public class DBSetupRunner implements CommandLineRunner {
  ...
  public void run(String... args) {
    ...
    try {
      ...
      System.out.println(
          "Created collection '" + spec.name() + "' in scope '" + spec.scopeName() + "' of bucket '" + bucket.name()
              + "'");
      Thread.sleep(1000);
    ...
    try {
      final String query = "CREATE PRIMARY INDEX default_profile_index ON " + props.getBucketName() + "._default."
          + CollectionNames.PROFILE;
    ...
    try {
      final QueryResult result = cluster.query(
          "CREATE INDEX default_profile_firstName_index ON " + props.getBucketName() + "._default."
              + CollectionNames.PROFILE + "(firstName)");
      Thread.sleep(1000);
    } catch (Exception e) {
